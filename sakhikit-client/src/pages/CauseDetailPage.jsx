          ...
          <div className="bg-white rounded-xl shadow-md p-6 border border-sakhi-100 h-fit">
            <div className="mb-4">
              ...
              <div className="w-full bg-sakhi-100 rounded-full h-3">
                <div
                  className="bg-sakhi-500 h-3 rounded-full transition-all"
                  style={{ width: `${percent}%` }}
                />
              </div>
              <p className="text-sm text-gray-500 mt-1">
                Goal: {cause.kitsNeeded} kits
              </p>
            </div>
            ...
            <div className="space-y-2 text-sm text-gray-600 mb-6">
              ...
              <p>📅 Deadline: {new Date(cause.deadline).toLocaleDateString('en-IN')}</p>
              {cause.location?.city && (
                <p>📍 {cause.location.city}, {cause.location.state}</p>
              )}
            </div>

            <button className="w-full bg-sakhi-700 text-white py-3 rounded-lg font-semibold hover:bg-sakhi-900 transition">
              Donate Now
            </button>
            <p className="text-xs text-gray-400 text-center mt-2">
              Stripe integration coming in Day 5
            </p>
          </div>
        </div>
      </div>
